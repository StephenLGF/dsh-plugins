...
function unwrapTransitions(payload) {
    ...
    for (const key of ['transitions', 'data', 'result']) {
        ...
        if (Array.isArray(value))
            return value;
        if (value && typeof value === 'object' && Array.isArray(value.transitions)) {
...
async function loadTransitions(config, itemKey) {
    ...
    return unwrapTransitions(payload).map(normalizeTransition).filter((value) => value !== null);
}
...
async function loadItems(config, assignee) {
    ...
    const items = rawItems.slice(0, maxItems).map(item => normalizeItem(item, priorityNames)).filter(item => (item.itemKey && item.title && item.status && !excludedStatuses.has(item.status))).map(item => ({ ...item, tomatoUrl: tomatoItemUrl(config, item.itemKey) }));
...
function loadItemsCached(config, assignee) {
    ...
    if (itemsCache?.key === key && itemsCache.expiresAt > Date.now())
        return Promise.resolve(itemsCache.value);
    if (itemsInFlight?.key === key)
        return itemsInFlight.promise;
    const promise = loadItems(config, assignee).then(value => {
        itemsCache = { key, expiresAt: Date.now() + ttl, value };
        return value;
    }).finally(() => {
        if (itemsInFlight?.promise === promise)
            itemsInFlight = null;
    });
...
async function loadFilters(config) {
    ...
    for (let page = 1; page <= 100; page += 1) {
        ...
        users.push(...values);
        if (!record.nextPageIndex || values.length === 0)
            break;
    }
    for (let page = 1; page <= 100; page += 1) {
...
async function loadSprints(config) {
    const sprints = [];
    for (let page = 1; page <= 100; page += 1) {
        const payload = await runJson(config, ['team', 'sprint', 'list', '--status', 'active,notStarted,completed', '--page', String(page), '--size', '100']);
        const values = Array.isArray(payload.sprints) ? payload.sprints : [];
        sprints.push(...values);
        if (!(Number(payload.nextPageIndex) > page) || !values.length)
            return { sprints };
    }
...
async function loadStoryItems(config, sprint, assignee) {
    ...
    for (let page = 1;; page += 1) {
        const payload = await runJson(config, ['team', 'item', 'search', '--iql', iql, '--fields', 'StoryPoint,assignee,sprint', '--page', String(page), '--size', '50']);
        ...
        for (const raw of values) {
            ...
            if (item.itemKey && isPlanningItem && itemSprints.includes(sprint))
                items.set(item.itemKey, { ...item, storyPoints: storyPointValue(fields.StoryPoint), tomatoUrl: tomatoItemUrl(config, item.itemKey) });
        }
        ...
        if (!more || !values.length)
            return { items: [...items.values()].slice(0, limit), truncated: items.size > limit };
        if (page * 50 >= limit)
            return { items: [...items.values()].slice(0, limit), truncated: true };
    }
}
function isCrossSite(request) {
    ...
    if (header('sec-fetch-site') === 'cross-site')
        return true;
    ...
    ctx.effect(() => webServer.register({
        ...
        async handler(request, response) {
            ...
            if (isCrossSite(request))
                return sendJson(response, 403, { error: '拒绝跨站修改请求' });
            ...
            if (!itemKey || points === null)
                return sendJson(response, 400, { error: '请输入非负有限故事点数值' });
            try {
                await runJson(config, ['team', 'item', 'edit', itemKey, '--values', JSON.stringify({ StoryPoint: points })]);
                const raw = await runJson(config, ['team', 'item', 'view', itemKey]);
                const fields = (raw.values ?? raw.value ?? raw);
                const actual = storyPointValue(fields.StoryPoint);
                if (actual !== points)
                    throw new Error('故事点保存后校验失败，请刷新后重试');
                itemsCache = null;
